...
module.exports.Dashing = function Dashing() {
  var dashing = {};
  dashing.root = path.resolve(__dirname, '../../../dash');
  ...
  dashing.mincer.assets_prefix = '/dash/assets';
  dashing.mincer.environment.appendPath(dashing.root + '/assets/javascripts');
  dashing.mincer.environment.appendPath(dashing.root + '/assets/stylesheets');
  dashing.mincer.environment.appendPath(dashing.root + '/assets/fonts');
  dashing.mincer.environment.appendPath(dashing.root + '/assets/images');
  dashing.mincer.environment.appendPath(dashing.root + '/widgets');
  ...
  app.get('/dash/events', dashing._protected, function(req, res) {
  ...
  app.get('/dash/:dashboard', dashing._protected, function(req, res) {
  ...
  app.get('/dash/views/:widget?.html', dashing._protected, function(req, res) {
    var widget = req.params.widget;
    res.sendfile([dashing.root, 'widgets', widget, widget + '.html'].join(path.sep));
  });

  app.post('/dash/widgets/:id', function(req, res) {
  ...
  dashing.start = function() {
    ...
    logger.info('Listening on http://0.0.0.0:' + dashing.port + '/dash' + (process.env.__daemon === 'false' ? ', CTRL+C to stop' : ''));
  }
  dashing.app = app;
  return dashing;
};
